describe Commands::Render do
  ...
  context 'with :markdown location ID' do
    # before do
    #   reset_tmp_dir create_src: true
    #   cp 'lib/bashly/templates/minimal.yml', "#{source_dir}/bashly.yml"
    # end

    # it 'outputs a markdown template' do
    #   allow(File).to receive(:deep_write)
    #   expect { subject.execute %w[render :markdown] }.to output_approval('cli/render/markdown')
    # end

    # it 'also generates markdown documents for each subcommand' do
    #   expect(File).to receive(:deep_write).exactly(3).times
    #   expect { subject.execute %w[render :markdown] }.to output_approval('cli/render/markdown')
    # end
  end
